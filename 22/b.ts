...
for (let i = 1; i < blocks.length + 1; i++) {
  let removedBricks: number[] = [i];
  while (true) {
    let breakLoop = true;
    for (let j = 1; j < blocks.length + 1; j++) {
      if (
        !removedBricks.includes(j) &&
        supportedBy[j] &&
        supportedBy[j].filter((d) => !removedBricks.includes(d)).length === 0
      ) {
        removedBricks.push(j);
        breakLoop = false;
      }
    }
    if (breakLoop) break;
  }
  console.log(i, removedBricks.length - 1);
  res += removedBricks.length - 1;
}
